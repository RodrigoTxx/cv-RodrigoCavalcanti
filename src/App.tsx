...
import { Mail, ExternalLink, Briefcase, GraduationCap, Award, Code, Globe, Bot, BadgeCheck, FolderGit2, Github, GitFork, Star, SkipForward, ChevronRight, List } from 'lucide-react'
...
function GridSnakes() {
  ...
  useEffect(() => {
    ...
    const onVisibility = () => {
      if (document.visibilityState === 'visible' && canvas.getBoundingClientRect().top < window.innerHeight) {
        start()
        return
      }
      stop()
...
function ReflectiveTypewriter({
  ...
  }, [phase, displayText, parsedContext.clean, reflections, currentReflection, hookParagraphs, parsedHookLines, currentHookParagraph, currentHookLine, getTypingDelay, onComplete])
          ...
          <div className="flex flex-col md:flex-row items-center gap-8 md:gap-12">
              ...
              <div className="relative w-40 h-40 md:w-48 md:h-48">
                ...
                <div className="absolute inset-2 rounded-full bg-gradient-theme-50 p-[2px]">
                  <div className="w-full h-full rounded-full overflow-hidden">
                    <img src="/foto-avatar-sm.webp" srcSet="/foto-avatar-sm.webp 192w, /foto-avatar.png 384w" sizes="(max-width: 768px) 160px, 192px" alt="Rodrigo Cavalcanti" className="w-full h-full object-cover" width={192} height={192} fetchPriority="high" />
                  </div>
                </div>
              </div>
              <motion.div
            ...
            >
              ...
              <h1 className="font-display text-3xl md:text-4xl lg:text-5xl font-bold tracking-tight mb-4 leading-tight">
                ...
                {t.greeting}
                <br />
                {lang === 'es' ? 'con ' : 'with '}<BeamPill>Geo <span className="opacity-60">+</span> AI <span className="opacity-60">+</span> RAG</BeamPill>
              </h1>
              ...
              <div className="flex flex-wrap justify-center md:justify-start gap-3">
                ...
                <a
                  href="https://github.com/rodrigoTxx"
                  target="_blank"
      ...
      <section id="experience" className="py-16 md:py-24 bg-muted/30" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 2000px' }}>
        <div className="max-w-5xl mx-auto px-6">
          ...
          {(() => {
            ...
            const items = t.experience.items as readonly ExperienceItem[]

            return (
              <div className="space-y-6">
                {items.map((item, index) => (
                  <AnimatedSection key={`${item.company}-${item.period}`} delay={0.08 + index * 0.06}>
                    <article className="p-6 rounded-2xl bg-card border border-border hover:border-primary/30 transition-colors duration-200">
                      <div className="flex flex-wrap items-start justify-between gap-3 mb-3">
                        ...
                        <span className="badge px-3 py-1 bg-primary/10 text-primary">{item.period}</span>
                      </div>
                      ...
                      {item.links && item.links.length > 0 && (
                        <div className="flex flex-wrap gap-3 mt-4 pt-4 border-t border-border/70">
      ...
      <section id="projects" className="py-16 md:py-24" style={{ contentVisibility: 'auto', containIntrinsicSize: 'auto 1500px' }}>
        <div className="max-w-5xl mx-auto px-6">
          ...
          {(() => {
            ...
            const currentProjects = (t.projects.items as readonly Project[]).slice(0, 3)
            ...
            const ProjectCard = ({ project, variant = 'default' }: { project: Project; variant?: 'default' | 'highlight' }) => {
              const isHighlight = variant === 'highlight'

              return (
                <div
                  className={`h-full p-6 rounded-2xl transition-colors duration-200 flex flex-col group ${
                    isHighlight
                ...
                >
                  <div className="flex items-start justify-between mb-3">
                    <h3 className="font-display text-xl font-bold transition-colors group-hover:text-primary">{project.title}</h3>
                    <div className="flex items-center gap-2">
                      <span className={`badge px-2 py-0.5 ${isHighlight ? 'bg-accent/10 text-accent' : 'bg-primary/10 text-primary'}`}>{project.badge}</span>
                  ...
                  <p className="text-sm text-muted-foreground mb-4">{parseBold(project.desc)}</p>
              ...
              <div className="mb-12">
                <div className="grid md:grid-cols-2 xl:grid-cols-3 gap-6">
                  {currentProjects.map((project, index) => (
                    <AnimatedSection key={project.title} delay={0.1 + index * 0.08}>
                      <ProjectCard project={project} variant={index === 1 ? 'highlight' : 'default'} />
                    </AnimatedSection>
                  ))}
                </div>
              </div>
                      ...
                      <div className="flex items-start justify-between">
                        <div>
                          ...
                          <p className="text-sm text-muted-foreground mt-1">
                            {item.desc}
                            {(() => {
                              const edu = item as typeof item & { projectLink?: string; projectLabel?: string }
                              return edu.projectLink ? (
                                <>
                                  {' '}
                                  <a
                                    href={`https://${edu.projectLink}`}
                                    target="_blank"
                                    rel="noopener noreferrer"
        ...
        <div className="relative z-10 max-w-5xl mx-auto px-6 text-center">
          ...
          <AnimatedSection delay={0.1}>
            <div className="flex flex-wrap justify-center gap-4">
              ...
              <a
                href="https://www.linkedin.com/in/rodrigo-cavalcantitx/"
                target="_blank"
                rel="noopener noreferrer"
